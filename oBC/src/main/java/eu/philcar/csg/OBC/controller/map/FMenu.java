...
//			updateUIUsingAppValues();
    ...
    private void UIHelper(int rentImage, int rentText, OnClickListener rentListener,
		...
		if(fuelListener!=null) {
			cancelIB.setEnabled(true);cancelIB.setOnClickListener(fuelListener);
		cancelTV.setOnClickListener(fuelListener);
		}
		else {cancelIB.setEnabled(false);
		...
		if(backListener!=null) {
			backIB.setEnabled(true);backIB.setOnClickListener(backListener);backIB.animate().alpha(1f);
		}else {
